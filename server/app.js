...
//Connect to Mongoose
mongoose.connect(connectionParameter, {
    ...
    .then(() => {
        console.log(`MongoDB connected on ${connectionParameter.substring(10, 15)}`)
    ...
    .catch(err => { 
        ...
        console.log(err);
        console.log('First DB connection attempt unsuccessful on: ', connectionParameter, ', trying next on: ', connectionParameter1);
        // Handling connection errors
        // Connect to replica
        mongoose.connect(connectionParameter1, {
            useNewUrlParser: true,
            useFindAndModify: false,
            useCreateIndex: true,
            useUnifiedTopology: true
        ...
        .then(() => {
            console.log(`MongoDB connected on ${connectionParameter1.substring(10, 15)}`)
            app.listen(PORT, () => console.log(`Server started on port ${PORT}`));
        })
        .catch(err => { 
            console.log(err);
